...
define(['ep', 'eventbus', 'backbone'],
  function (ep, EventBus, Backbone) {
    ...
    var categoryItemPageModel = Backbone.Model.extend({
      getUrl: function (uri) {
        return ep.app.config.cortexApi.path + ep.ui.decodeUri(uri) + '?zoom=element, element:availability,element:definition,element:definition:assets:element,element:price,element:rate, element:addtocartform';
      },
      parse: function (response) {
        var categoryObj = {};
        ...
        for (var i = 0; i < itemArrayLen; i++) {
          ...
          if (addToCartFormAction){
            itemObj.addtocart.actionlink = jsonPath(itemArray[i], "$._addtocartform..links[?(@.rel='addtodefaultcartaction')].uri")[0];
          }
